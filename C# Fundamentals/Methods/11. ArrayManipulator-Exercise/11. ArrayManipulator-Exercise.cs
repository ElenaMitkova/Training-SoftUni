...
while (command != "end")
{
    ...
    switch (commandArray[0])
    {
        case "exchange":
            ...
            break;
        case "max":
            string maxType = commandArray[1];
            PrintMaxIndex(numbers, maxType);
            break;
        case "min":
            string minType = commandArray[1];
            PrintMinIndex(numbers, minType);
            break;
        case "first":
            int length = int.Parse(commandArray[1]);
            string firstType = commandArray[2];
            PrintFirstElements(numbers, length, firstType);
            break;
        case "last":
            int lastCount = int.Parse(commandArray[1]);
            string lastType = commandArray[2];
