...
for (int i = 0; i < n; i++)
{
    ...
    double value = 0;
    Dragon dragon = new Dragon() { Type = parameters[0], Name = parameters[1], Damage = double.TryParse(parameters[2], out value) ? value : 45, Health = double.TryParse(parameters[3], out value) ? value : 250, Armour = double.TryParse(parameters[4], out value) ? value : 10, };
...
var groupedDragons = dragons.GroupBy(d => d.Type, d => d, (key, d) => new GroupedDragon() { Type = key, Dragons = d.ToList() });
